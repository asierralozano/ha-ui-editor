...
class BaseNode(NodeObject):
    ...
    **Inherited from:** :class:`NodeGraphQt.NodeObject`
    ...
    def add_long_text_input(self, name, label='', text='', tab=None, multi_line=False):
        """
        Creates a custom property with the :meth:`NodeObject.create_property`
        function and embeds a :class:`PySide2.QtWidgets.QLineEdit` widget
        into the node.
    ...
    def add_input(self, name='input', multi_input=False, display_name=True,
        ...
        #     port_kwargs.append(painter_func)
        view = self.view.add_port(**port_kwargs)
        ...
        port.model.limited_type = limited_type
...
class TemplateBaseNode(BaseNode):
    ...
    def __init__(self, theme, template_type, template):
        ...
        super(TemplateBaseNode, self).__init__()

        self._model.type_ = self.type_()
        self._view.type_ = self.type_()

    def type_(cls):
        return f"{cls._theme}.{cls._template_type}.{cls._template}"
    ...
    def set_model(self, model):
        ...
        Args:
            model (NodeGraphQt.base.model.NodeModel): node model object.
        """
        super(TemplateBaseNode, self).set_model(model)
        self._model.type_ = self.type_()
